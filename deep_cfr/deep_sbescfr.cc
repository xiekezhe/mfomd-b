...
void actor(const Game& game, const DeepCFRConfig& config, int num,
  ...
  DeepSbESCFRSolver solver(game, config, value_evals, global_value_evals,
                           policy_evals, current_policy_evals,true, &rng);
...
std::string SaveAndLoad(const DeepCFRConfig& config,
                        ...
                        int value_or_policy, Player player, int device_id) {
...
void SyncModels(const DeepCFRConfig& config, DeviceManager* device_manager,
                int step, int value_or_policy, Player player, int device_id) {
  if (value_or_policy!=2) {
    for (auto& device : device_manager->GetAll()) {
...
LearningInfo learn_imp_(const Game& game, const DeepCFRConfig& config,
                        ...
                        int value_current_policy, Player player, int memory_size,
    ...
    if (trajectory) {
      ...
      if (value_current_policy==0) {
        ...
        for (++iter; iter != trajectory->states.end(); ++iter, ++next) {
          ...
          iter->next_opponent_reach = next->opponent_reach;
      ...
      }else if (value_current_policy==2) {
        for (ReplayNode& n : trajectory->states) {
          SPIEL_CHECK_EQ(n.current_player, player);
          PolicyReplayNode policy_node{n.info_str,n.information, n.current_player,
                                       n.legal_actions, n.action_index,
                                       n.weight};
   ...
   /* if (value_or_policy) {
    ...
    for (int train_step = 0; train_step < train_steps; train_step++) {
       std::vector<CFRNetModel::InferenceOutputs> target_outputs;
        std::vector<CFRNetModel::InferenceOutputs> target_policy_outputs;
    ...
    if (value_current_policy==0) {
        ...
        target_policy_outputs =
            learn_model->InfCurrentTargetPolicy(player, target_inference_inputs);
         /*target_outputs = learn_model->GetCFRTabularValues(
             player, target_inference_inputs);
         target_policy_outputs = learn_model->GetCFRTabularPolicies(
             player, target_inference_inputs);*/
        for (int i = 0; i != data.size(); ++i) {
          const auto& node = data[i];
          ...
          if (node.next_action_index != -1) {
           std::vector<double> sub_values(node.next_legal_actions.size());
            ...
            std::vector<double> target_policy(node.next_legal_actions.size(),1.0 / (node.next_legal_actions.size()));
            if(step!=1)
           { target_policy = target_policy_outputs[i].value;}
            target_value[0] = cfrplus_backward_update_regret(
          alpha,
          node.next_opponent_reach,
          ref_value, sub_values, target_policy, step);
          int sdaw =0;


          }
          target_value[0] = target_value[0]*node.value_weight;
          target_value[0] += node.value*node.value_weight;
          ...
          train_inputs.emplace_back(CFRNetModel::TrainInputs{
             node.info_str, legal_actions,legal_actions, node.information, target_value, 1});
        }
      }else if(value_current_policy==1) {
        std::vector<ReplayNode> data =
            replay_buffer.GetNext(rng, train_batch_size);
        // set target.
        for (int i = 0; i != data.size(); ++i) {
       ...
       // target_policy_outputs =
        //    learn_model->InfCurrentTargetPolicy(player, target_inference_inputs);
         for (int i = 0; i != data.size(); ++i) {
          const auto& node = data[i];
          std::vector<double> target_value(1, 0);
           if(node.current_or_average==true)
         {
            std::vector<double> sub_values(node.legal_actions.size());
            std::vector<double> next_policy(node.legal_actions.size());
            std::vector<double> next_regret(node.legal_actions.size());
            std::vector<double> target_policy(node.legal_actions.size(),1.0 / (node.legal_actions.size()));
            std::vector<double>& ref_value = target_outputs[i].value;
            SPIEL_CHECK_FALSE(node.legal_actions.empty());
           // SPIEL_CHECK_EQ(node.cfrvalue.size(), target_policy_outputs[i].value.size());
            if(step!=1)
           { target_policy = node.policy;}
         ...
         next_regret[aidx] = next_policy[aidx] =
          std::max(sub_values[aidx] -  target_value[0], 0.0);
          }
           CFRInfoStateValues next_info_state(node.legal_actions, 0.00000001);
          ...
          train_inputs.emplace_back(CFRNetModel::TrainInputs{
              node.info_str,node.legal_actions,node.legal_actions, node.information, next_policy, 1});
          
          }
         
        }

      }else if(value_current_policy==2&&config.use_policy_net == true) {
        ...
        for (int i = 0; i != data.size(); ++i) {
          ...
          train_inputs.emplace_back(CFRNetModel::TrainInputs{
               node.info_str,node.legal_actions,node.legal_actions, node.information, value, node.weight});
        }
      }
      {
        auto start = std::chrono::system_clock::now();
      if (value_current_policy==0) {
         ...
         //   loss += learn_model->TrainValueTabular(player, train_inputs);
        }else if (value_current_policy==1) {
         loss += learn_model->TrainCurrentPolicy(player, train_inputs,
                                           config.learning_rate);
         // loss += learn_model->TrainCurrentPolicyTabular(player, train_inputs);
        } else if (value_current_policy==2&&config.use_policy_net == true) {
        ...
        if (config.verbose) {
          ...
          if (train_step % (train_steps / 2) == 0)
            std::cout << "train used " << sec << "s " << train_inputs.size() / sec
                      << " state/s" << std::endl;
        }
      }
    ...
    if (step % config.sync_period == 0 && value_current_policy==0) {
      learn_model->SyncValue(player);
    }else if(step % config.sync_period == 0 && value_current_policy==1) {
      learn_model->SyncCurrentPolicy(player);
    }
  }
  ...
  if (config.sync_by_restore) {
    std::string checkpoint_path = SaveAndLoad(
        config, device_manager, -1, value_current_policy, player, device_id);
    if (config.verbose) {
      logger.Print("Checkpoint saved: %s", checkpoint_path);
    }
  }
  if (config.sync_by_copy) {
    SyncModels(config, device_manager, step, value_current_policy, player,
               device_id);
  }
    ...
    for (int player = 0; player < 2; ++player) {
      ...
      auto policy_trajectory_queue = policy_trajectory_queues[player];
      {
        auto learn_info = learn_imp_(
            ...
            value_trajectory_queue, value_replay_buffer[player],policy_replay_buffer[player], 0, player,
            config.memory_size, config.cfr_batch_size, config.global_value_train_steps,
      ...
      }
      {
        auto learn_info = learn_imp_(
            game, config, device_manager, device_id, logger, rng, step,
            value_trajectory_queue, value_replay_buffer[player], policy_replay_buffer[player],1, player,
      ...
      }
      {
        ...
        auto learn_info = learn_imp_(
            game, config, device_manager, device_id, logger, rng, step,
            policy_trajectory_queue, value_replay_buffer[player],policy_replay_buffer[player], 2, player,
            config.policy_memory_size, config.cfr_batch_size,
            config.policy_train_steps, config.train_batch_size, alpha,
            new_cfr_value, true, stop);
...
bool deep_cfr(DeepCFRConfig config, StopToken* stop) {
  ...
  std::unordered_map<std::string, GameParameters> game_parameters = {
      ...
      {"leduc_poker18", universal_poker::Leduc18PokerParameters()},
  ...
  for (int i = 0; i < config.actors; ++i) {
    actors.emplace_back([&, i]() {
      actor(*game, config, i,
            ...
            {&policy_0_trajectory_queue, &policy_1_trajectory_queue},
            {&value_0_trajectory_queue, &value_1_trajectory_queue},
